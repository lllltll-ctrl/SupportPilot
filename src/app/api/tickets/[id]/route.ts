...
export async function GET(
  ...
  try {
    ...
    const conversation = await conversationRepository.findByTicketId(ticketId);
    const messages = conversation
      ? await messageRepository.findByConversationId(conversation.id)
      : [];
    const actions = await actionLogRepository.findByTicketId(ticketId);
...
export async function PATCH(
  ...
  try {
    ...
    if (body.status) {
      await ticketRepository.updateStatus(ticketId, body.status);

      // Auto-follow-up: when a ticket is resolved, schedule a follow-up message
      if (body.status === 'resolved') {
        const conv = await conversationRepository.findByTicketId(ticketId);
        if (conv) {
    ...
    if (body.assigned_agent_id !== undefined) {
      await ticketRepository.assignAgent(ticketId, body.assigned_agent_id);
    }

    if (body.priority) {
      await ticketRepository.updatePriority(ticketId, body.priority);
    }

    if (body.satisfaction_rating && body.satisfaction_rating >= 1 && body.satisfaction_rating <= 5) {
      await ticketRepository.updateSatisfaction(ticketId, body.satisfaction_rating);
    }
